...
export async function POST(req: NextRequest) {
  try {
    ...
    // CPF, identidade, email — normaliza antes de comparar (evita mismatch com/sem máscara)
    const normalizeDoc   = (s: string) => String(s || '').replace(/\D/g, '');
    const normalizeEmail = (s: string) => String(s || '').trim().toLowerCase();

    const normalizedValue = field === 'email' ? normalizeEmail(cleanValue) : normalizeDoc(cleanValue);
    if (!normalizedValue) return NextResponse.json({ duplicate: false });

    // Busca ampla e compara normalizado em JS
    ...
    const match = (data || []).find(s => {
      if (excludeId && s.id === excludeId) return false;
      if (field === 'cpf')        return normalizeDoc(s.cpf || '')        === normalizedValue;
      if (field === 'identidade') return normalizeDoc(s.identidade || '')  === normalizedValue;
      if (field === 'email')      return normalizeEmail(s.email || '')     === normalizedValue;
      return false;
    });
