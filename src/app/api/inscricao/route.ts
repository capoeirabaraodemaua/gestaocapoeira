...
async function ensureNullableColumns() {
  ...
  const newCols = [
    ...
    // Garante que created_at tenha DEFAULT now() para novos registros
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Normaliza CPF/identidade: remove tudo que não é dígito para comparação uniforme
    function normalizeDoc(s: unknown): string {
    ...
    const cpfNorm       = normalizeDoc(payload.cpf);
    const identNorm     = normalizeDoc(payload.identidade);
    const emailNorm     = normalizeEmail(payload.email);

    // Verificar duplicata por CPF, identidade ou email — compara dígitos normalizados
    // Busca todos os candidatos que possam bater (pela primeira letra do CPF/identidade)
    // e compara normalizado no JS para evitar mismatch de formatação (com/sem máscara)
    const cpfCheck    = cpfNorm.length >= 6;
    const identCheck  = identNorm.length >= 4;
    const emailCheck  = emailNorm.length > 3 && emailNorm.includes('@');

    if (cpfCheck || identCheck || emailCheck) {
      // Busca ampla: traz todos para comparar normalizado em JS
      ...
      const dup = (allStudents || []).find(s => {
        if (cpfCheck    && normalizeDoc(s.cpf)           === cpfNorm)    return true;
        if (identCheck  && normalizeDoc(s.identidade)    === identNorm)  return true;
        if (emailCheck  && normalizeEmail(s.email)       === emailNorm)  return true;
        return false;
      });
      ...
      if (dup) {
        let campo = '';
        let fieldKey = 'cpf';
        if (cpfCheck    && normalizeDoc(dup.cpf)        === cpfNorm)   { campo = `CPF ${payload.cpf}`; fieldKey = 'cpf'; }
        else if (identCheck && normalizeDoc(dup.identidade) === identNorm) { campo = `Numeração Única/RG "${payload.identidade}"`; fieldKey = 'identidade'; }
        else                                                              { campo = `e-mail "${payload.email}"`; fieldKey = 'email'; }
        return NextResponse.json(
          {
            error: `Aluno já cadastrado com ${campo}: ${dup.nome_completo}.`,
            duplicate: true,
            field: fieldKey,
            existing_id: dup.id,
          },
    ...
    if (nomeRaw) {
      ...
      if (candidates && candidates.length > 0) {
        ...
        if (dup) {
          return NextResponse.json(
            {
              error: `Aluno já cadastrado! Já existe um registro com o nome "${dup.nome_completo}". Se você já se cadastrou antes, acesse a Área do Aluno para completar seus dados em vez de criar um novo cadastro.`,
              duplicate: true,
              field: 'nome',
            },
            { status: 409 }
    ...
    // Garante created_at — não depende do DEFAULT do banco (pode estar ausente em schemas antigos)
    if (!safePayload.created_at) {
