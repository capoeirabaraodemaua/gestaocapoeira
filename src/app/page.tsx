...
import { graduacoesInfantil, graduacoesAdulta, getCordaColors, nomenclaturaGraduacao } from '@/lib/graduacoes';
...
export default function Home() {
  ...
  async function handleAdminAccess() {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        const ALL_NUCLEOS = ['edson-alves', 'ipiranga', 'saracuruna', 'vila-urussai', 'jayme-fichman', 'academia-mais-saude', 'geral'];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (honeypot) return;
    // Trava de duplo-submit: se já está processando, ignora clique extra
    if (loading) return;
    ...
    // Re-verifica duplicatas em tempo real no submit (não confia em estado onBlur que pode estar desatualizado)
    // Isso resolve o caso: aluno excluído e tentando re-cadastrar — o estado antigo ficava travando
    setLoading(true);
    const reCheckFields: Array<{ field: 'cpf' | 'identidade' | 'email' | 'nome'; value: string }> = [];
    if (form.nome_completo.trim()) reCheckFields.push({ field: 'nome', value: form.nome_completo.trim() });
    if (form.cpf?.trim())         reCheckFields.push({ field: 'cpf', value: form.cpf.trim() });
    if (form.identidade?.trim())  reCheckFields.push({ field: 'identidade', value: form.identidade.trim() });
    if (form.email?.trim())       reCheckFields.push({ field: 'email', value: form.email.trim() });

    const freshErrors: Record<string, string> = {};
    await Promise.all(reCheckFields.map(async ({ field, value }) => {
      try {
        ...
        if (r.ok) {
          ...
          if (d.duplicate && d.message) freshErrors[field] = d.message;
          else if (d.error) freshErrors[field] = d.error;
        }
      } catch {}
    }));

    if (Object.keys(freshErrors).length > 0) {
      setDuplicateErrors(freshErrors);
      setLoading(false);
      const firstField = Object.keys(freshErrors)[0];
      setTimeout(() => {
        const el = document.querySelector(`[name="${firstField === 'nome' ? 'nome_completo' : firstField}"]`) as HTMLElement | null;
        el?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        el?.focus();
      }, 50);
      return;
    }
    // Nenhuma duplicata encontrada — limpa erros antigos e prossegue
    setDuplicateErrors({});
...
_Associação Cultural de Capoeira Barão de Mauá_`
  ...
  return (
    <>
            ...
            {!bgCpfOk ? (
              <>
                ...
                <button
                  ...
                  style={{ background: '#064E3B', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 20px', fontWeight: 700, cursor: 'pointer', width: '100%' }}
                >
                  {t('bg_verify_btn')}
                </button>
              </>
              ...
              <>
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      // API already saves config and returns a fresh signed URL
                      if (json.url) setBgUrl(json.url);
                ...
                <button
                  ...
                  style={{ background: '#064E3B', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 20px', fontWeight: 700, cursor: bgUploading ? 'not-allowed' : 'pointer', width: '100%', marginBottom: 8, opacity: bgUploading ? 0.7 : 1 }}
                >
      ...
      <div className="container" style={{ marginTop: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 0, flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                background: activeSection === 'ficha' ? '#064E3B' : 'rgba(6,78,59,0.10)',
                border: 'none',
                borderRadius: '10px 10px 0 0',
                padding: '11px 22px',
                color: activeSection === 'ficha' ? '#fff' : '#064E3B',
                fontWeight: 800,
                fontSize: '0.88rem',
            ...
            <div className="form-grid">
              <div className="form-group full-width">
                ...
                {checkingDuplicate.nome && (
                  <span style={{ fontSize: '0.78rem', color: '#6ee7b7' }}>{t('checking_name')}</span>
                )}
              ...
              <div className="form-group">
                ...
                />
                {checkingDuplicate.cpf && <span style={{ fontSize: '0.78rem', color: '#6ee7b7' }}>{t('checking_field')}</span>}
                {duplicateErrors.cpf && (
              ...
              <div className="form-group">
                ...
                />
                {checkingDuplicate.identidade && <span style={{ fontSize: '0.78rem', color: '#6ee7b7' }}>{t('checking_field')}</span>}
          ...
          {menorDeIdade && (
            <div className="form-section" style={{ border: '2px solid #dc2626', padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px 20px' }}>
                <div style={{ fontFamily: 'Georgia, serif', fontSize: '0.9rem', lineHeight: 1.8, textAlign: 'justify', marginBottom: 20, color: 'var(--text-primary)', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <p>Eu, responsável legal pelo menor inscrito, autorizo sua participação nas atividades de capoeira realizadas pela <strong>Associação Cultural de Capoeira Barão de Mauá</strong>, declarando estar ciente das práticas físicas envolvidas.</p>
                  ...
                  <p>Autorizo, de forma livre, informada e inequívoca, a coleta e o tratamento dos dados pessoais do menor e do responsável, incluindo nome completo, CPF, telefone e/ou e-mail e informações sociais, bem como dados de localização (GPS), coletados pontualmente no momento do registro de presença, exclusivamente para fins de confirmação de participação nas atividades. <em>Não é realizado monitoramento contínuo de localização.</em></p>
                  <div style={{ background: 'rgba(220,38,38,0.05)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8, padding: '12px 16px' }}>
                    <p style={{ fontWeight: 700, marginBottom: 8, fontSize: '0.85rem', fontFamily: 'sans-serif' }}>🔒 USO DE DADOS PESSOAIS (LGPD)</p>
                    <p style={{ marginBottom: 8 }}>Os dados serão utilizados para: gestão e cadastro de alunos, controle de presença e frequência, comunicação com responsáveis, organização das atividades e elaboração de relatórios sociais e institucionais.</p>
                    <p style={{ marginBottom: 8 }}>Os dados serão utilizados exclusivamente pela administração da Associação Cultural de Capoeira Barão de Mauá, com acesso restrito à equipe responsável, não sendo divulgados publicamente.</p>
                    <p style={{ marginBottom: 8 }}>A associação adota medidas técnicas e administrativas para proteger os dados pessoais contra acessos não autorizados e situações acidentais ou ilícitas.</p>
                    <p>O tratamento dos dados será realizado em conformidade com o <strong>Estatuto da Criança e do Adolescente (ECA)</strong> e a <strong>Lei Geral de Proteção de Dados (LGPD)</strong>. Este consentimento poderá ser revogado a qualquer momento mediante solicitação do responsável.</p>
                  </div>
                </div>
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <select value={graduacao} onChange={(e) => setGraduacao(e.target.value)}>
                  ...
                  {(tipoGraduacao === 'infantil' ? graduacoesInfantil : graduacoesAdulta).map(g => (
              ...
              {graduacao && (
                <div className="form-group full-width">
                  ...
                  <div style={{ height: 24, borderRadius: 12, overflow: 'hidden', display: 'flex', border: '1px solid var(--border)', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
                    {getCordaColors(graduacao).map((color, i) => (
                      <div key={i} style={{ flex: 1, height: '100%', background: color }} />
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)' }}>{graduacao}</span>
                    {nomenclaturaGraduacao[graduacao] && (
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '2px 10px', borderRadius: 20, background: 'rgba(6,78,59,0.1)', color: '#064E3B', border: '1px solid rgba(6,78,59,0.2)' }}>
                        {nomenclaturaGraduacao[graduacao]}
                      </span>
                    )}
                  </div>
          ...
          <div className="form-section" style={{ borderTopLeftRadius: 0, marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{ background: '#064E3B', border: 'none', color: '#fff', padding: '10px 20px', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: '0.88rem', whiteSpace: 'nowrap' }}
              >
          ...
          <div className="success-card" style={{ maxWidth: 420, width: '95%', padding: '28px 24px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              {successData.inscricao_numero != null && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#064E3B', borderRadius: 8, padding: '6px 16px' }}>
            ...
            <button
              ...
              style={{ width: '100%', padding: '11px', background: '#F3F4F6', border: '1px solid var(--border)', color: 'var(--text-secondary)', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: '0.88rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8 }}
            >
            ...
            {successData?.student_id && (
              <div style={{ marginTop: 8, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 12, padding: '16px 16px 12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#064E3B', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                {successDisplayId && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#dbeafe', borderRadius: 8, padding: '6px 10px', marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: '0.76rem', color: '#064E3B', fontWeight: 600 }}>ID: </span>
                    <span style={{ fontSize: '0.88rem', fontWeight: 900, color: '#064E3B', letterSpacing: '0.05em' }}>{successDisplayId}</span>
                  </div>
                )}
                ...
                {!criarContaUsername && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <button
                      ...
                      style={{ background: criarContaLoading || criarContaPassword.length < 6 ? '#9ca3af' : '#064E3B', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontWeight: 700, fontSize: '0.85rem', cursor: criarContaLoading || criarContaPassword.length < 6 ? 'not-allowed' : 'pointer' }}
                    >
      ...
      {/* ── Rodapé ── */}
      <footer style={{ marginTop: 32, paddingBottom: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', textAlign: 'center', lineHeight: 1.6 }}>
          © 2026 Ginga Gestão · Desenvolvido por Andre Ferreira · Todos os direitos reservados.
        </div>
        <a
          href="https://api.whatsapp.com/send?phone=5521966102513"
          target="_blank"
          rel="noopener noreferrer"
          title="WhatsApp: (21) 96610-2513"
          style={{
      ...
      <button
        ...
        style={{
          ...
          background: '#064E3B',
          color: '#fff', border: '1.5px solid rgba(255,255,255,0.2)', borderRadius: '8px',
          padding: '8px 14px', fontSize: '11px', cursor: 'pointer',
          zIndex: 9999, backdropFilter: 'blur(4px)', letterSpacing: '0.03em',
          fontWeight: 700, boxShadow: '0 2px 12px rgba(6,78,59,0.35)',
        }}
      >
        🔒 {t('admin_panel_title')}
      </button>
      ...
      {adminModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}
          ...
          <div style={{ background: 'linear-gradient(160deg,#064E3B,#065f46)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 16, padding: '28px 26px', width: 520, maxWidth: '96vw', display: 'flex', flexDirection: 'column', gap: 14, boxShadow: '0 24px 80px rgba(0,0,0,0.6)', maxHeight: '92vh', overflowY: 'auto' }}>
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  ...
                  <div onClick={() => { setAdminUser('admin'); setTimeout(() => document.getElementById('adminPassInput')?.focus(), 50); }}
                    style={{ background: 'rgba(6,78,59,0.3)', border: `1.5px solid ${adminUser === 'admin' ? '#6ee7b7' : 'rgba(255,255,255,0.3)'}`, borderRadius: 10, padding: '10px 8px', cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s', boxShadow: adminUser === 'admin' ? '0 0 0 2px rgba(6,78,59,0.4)' : 'none' }}>
                    ...
                    <div style={{ color: '#6ee7b7', fontWeight: 800, fontSize: '0.7rem', lineHeight: 1.2 }}>Admin Geral</div>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 12 }}>
                  ...
                  {adminErro && (
                  <div style={{ marginTop: 8, background: 'rgba(220,38,38,0.12)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 8, padding: '8px 12px', color: '#f87171', fontSize: '0.78rem', fontWeight: 600, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <span>⚠ {adminErro}</span>
                    {(adminErro.includes('Aguarde') || adminErro.includes('Bloqueado')) && (
                      <button
                        type="button"
                        onClick={() => { setLoginState(0, 0); setAdminErro(''); }}
                        style={{ background: 'rgba(251,191,36,0.15)', border: '1px solid rgba(251,191,36,0.4)', color: '#fbbf24', borderRadius: 6, padding: '5px 12px', fontSize: '0.73rem', fontWeight: 700, cursor: 'pointer', alignSelf: 'flex-start' }}
                      >
                  ...
                  <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                    ...
                    <button onClick={handleAdminAccess} disabled={adminLoading}
                      style={{ flex: 2, padding: '10px', borderRadius: 9, background: '#064E3B', border: 'none', color: '#fff', cursor: adminLoading ? 'not-allowed' : 'pointer', fontSize: '0.9rem', fontWeight: 700, opacity: adminLoading ? 0.7 : 1 }}>
            ...
            {adminScreen === 'change' && (
              <>
                ...
                <div style={{ background: 'rgba(6,78,59,0.15)', border: '1px solid rgba(6,78,59,0.3)', borderRadius: 8, padding: '8px 12px', color: '#6ee7b7', fontSize: '0.75rem' }}>
                ...
                <button disabled={manageSaving} onClick={async () => {
                  ...
                  setManageSaving(false);
                }} style={{ padding: '10px', borderRadius: 9, background: '#064E3B', border: 'none', color: '#fff', fontWeight: 700, fontSize: '0.9rem', cursor: manageSaving ? 'wait' : 'pointer', opacity: manageSaving ? 0.7 : 1 }}>
