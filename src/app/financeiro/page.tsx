...
export default function FinanceiroPage() {
  ...
  // Uniform request form — structured
  const [unifFormData, setUnifFormData] = useState<Record<string, any>>({ tipo_uniforme: 'camisa', quantidade: 1 });
  const [showUnifForm, setShowUnifForm] = useState(false);
  const [unifSaving, setUnifSaving] = useState(false);
  const [unifMsg, setUnifMsg] = useState('');
  ...
  const addUniforme = async () => {
    if (!ficha) return;
    setUnifSaving(true); setUnifMsg('');
    const tipo = unifFormData.tipo_uniforme || 'camisa';
    let descricao = '', tamanho = '';
    if (tipo === 'camisa') {
      if (!unifFormData.tamanho?.trim()) { setUnifMsg('Informe o tamanho da camisa.'); setUnifSaving(false); return; }
      descricao = `Camisa${unifFormData.cor ? ' ' + unifFormData.cor : ''}`;
      tamanho = unifFormData.tamanho;
    } else if (tipo === 'calca') {
      descricao = `Calça de Capoeira${unifFormData.cor ? ' — ' + unifFormData.cor : ''}`;
      const medidas = [unifFormData.cintura && `Cintura: ${unifFormData.cintura}cm`, unifFormData.altura_calca && `Altura: ${unifFormData.altura_calca}cm`, unifFormData.gaviao && `Gavião: ${unifFormData.gaviao}cm`].filter(Boolean).join(' | ');
      tamanho = medidas;
      if (!medidas) { setUnifMsg('Informe ao menos uma medida.'); setUnifSaving(false); return; }
    } else if (tipo === 'graduacao') {
      const subtipo = (unifFormData.subtipo_graduacao as string) || 'Corda';
      if (!unifFormData.tamanho?.trim()) { setUnifMsg('Informe o tamanho.'); setUnifSaving(false); return; }
      descricao = subtipo;
      tamanho = unifFormData.tamanho;
    } else {
      if (!unifFormData.descricao_outro?.trim()) { setUnifMsg('Informe a descrição do item.'); setUnifSaving(false); return; }
      descricao = unifFormData.descricao_outro.trim();
      tamanho = unifFormData.tamanho || '';
    }
    const item: UniformeItem = {
      ...
      quantidade: unifFormData.quantidade || 1,
      valor_unitario: 0,
      status: 'solicitado',
      data_solicitacao: new Date().toISOString().slice(0, 10),
      ...(tipo === 'graduacao' ? { tipo_uniforme: 'corda', subtipo_graduacao: (unifFormData.subtipo_graduacao as string) || 'Corda' } : {}),
    } as any;
    const updated = { ...ficha, uniformes: [...ficha.uniformes, item] };
    setFicha(updated);
    await saveFicha(updated);
    setUnifFormData({ tipo_uniforme: 'camisa', quantidade: 1 });
    setShowUnifForm(false);
    setUnifMsg('✅ Solicitação enviada!');
    setUnifSaving(false);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#064E3B,#065f46,#064E3B)', color: '#fff', fontFamily: 'Inter,sans-serif', padding: '0 0 60px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(220,38,38,0.15),rgba(30,58,138,0.2))', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#064E3B,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', flexShrink: 0 }}>💰</div>
        <div>
          ...
          <div style={{ maxWidth: 380, margin: '40px auto' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button onClick={handleLogin} disabled={loadingLogin}
                style={{ padding: '13px', background: 'linear-gradient(135deg,#064E3B,#059669)', border: 'none', color: '#fff', borderRadius: 10, cursor: 'pointer', fontWeight: 800, fontSize: '1rem', opacity: loadingLogin ? 0.6 : 1 }}>
              ...
              <div style={cardStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
                  <span style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.55)' }}>
                    {ficha.uniformes.filter(u => u.status === 'solicitado').length > 0
                      ? `${ficha.uniformes.filter(u => u.status === 'solicitado').length} solicitação(ões) pendente(s)`
                      : 'Nenhuma solicitação pendente'}
                  </span>
                  <button onClick={() => { setShowUnifForm(v => !v); setUnifFormData({ tipo_uniforme: 'camisa', quantidade: 1 }); setUnifMsg(''); }}
                    style={{ padding: '7px 16px', background: showUnifForm ? 'rgba(255,255,255,0.08)' : 'rgba(217,119,6,0.25)', border: `1px solid ${showUnifForm ? 'rgba(255,255,255,0.15)' : 'rgba(217,119,6,0.5)'}`, color: showUnifForm ? 'rgba(255,255,255,0.6)' : '#fbbf24', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem' }}>
                    {showUnifForm ? '✕ Cancelar' : '+ Nova solicitação'}
                  </button>
                </div>

                {/* Form */}
                {showUnifForm && (
                  <div style={{ background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.25)', borderRadius: 12, padding: '16px', marginBottom: 16 }}>
                    {/* Type tabs */}
                    <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
                      {([['camisa','👕 Camisa'],['calca','👖 Calça'],['graduacao','🎗️ Graduação']] as const).map(([k, lbl]) => (
                        <button key={k} onClick={() => setUnifFormData({ tipo_uniforme: k, quantidade: 1, subtipo_graduacao: 'Corda' })}
                          style={{ padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.78rem', border: 'none',
                            background: (unifFormData.tipo_uniforme || 'camisa') === k ? '#d97706' : 'rgba(255,255,255,0.08)',
                            color: (unifFormData.tipo_uniforme || 'camisa') === k ? '#fff' : 'rgba(255,255,255,0.55)' }}>
                          {lbl}
                        </button>
                      ))}
                    </div>
                    ...
                    {/* CAMISA */}
                    {(unifFormData.tipo_uniforme || 'camisa') === 'camisa' && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Tamanho (livre)</label>
                          <input value={unifFormData.tamanho || ''} onChange={e => setUnifFormData((p: any) => ({ ...p, tamanho: e.target.value }))} placeholder="PP, P, M, G, GG..."
                            style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Cor (livre)</label>
                          <input value={unifFormData.cor || ''} onChange={e => setUnifFormData((p: any) => ({ ...p, cor: e.target.value }))} placeholder="Branca, Preta, Azul..."
                            style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Quantidade</label>
                          <input type="number" min={1} value={unifFormData.quantidade || 1} onChange={e => setUnifFormData((p: any) => ({ ...p, quantidade: parseInt(e.target.value) || 1 }))}
                            style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
                        </div>
                      </div>
                    )}
                    ...
                    {/* CALÇA */}
                    {(unifFormData.tipo_uniforme || 'camisa') === 'calca' && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                        <div style={{ gridColumn: '1 / -1' }}>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Cor</label>
                          <select value={unifFormData.cor || 'branca'} onChange={e => setUnifFormData((p: any) => ({ ...p, cor: e.target.value }))}
                            style={{ width: '100%', padding: '8px 10px', background: 'rgba(30,30,30,0.8)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }}>
                            <option value="branca">⬜ Branca (oficial)</option>
                            <option value="preta">⬛ Preta (treino)</option>
                            <option value="azul">🟦 Azul (treino)</option>
                            <option value="cinza">🩶 Cinza (treino)</option>
                            <option value="verde">🟩 Verde (treino)</option>
                            <option value="outra">Outra cor (treino)</option>
                          </select>
                        </div>
                        {([['cintura','Cintura (cm)','80'],['altura_calca','Altura (cm)','100'],['gaviao','Gavião (cm)','35']] as const).map(([k,lbl,ph]) => (
                          <div key={k}>
                            <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>{lbl}</label>
                            <input type="number" value={(unifFormData as any)[k] || ''} onChange={e => setUnifFormData((p: any) => ({ ...p, [k]: e.target.value }))} placeholder={ph}
                              style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
                          </div>
                        ))}
                        <div>
                    ...
                    {/* GRADUAÇÃO */}
                    {(unifFormData.tipo_uniforme || 'camisa') === 'graduacao' && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                        <div style={{ gridColumn: '1 / -1' }}>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Tipo</label>
                          <div style={{ display: 'flex', gap: 6 }}>
                            {(['Corda', 'Cordel', 'Cordão'] as const).map(opt => (
                              <button key={opt} type="button"
                                onClick={() => setUnifFormData((p: any) => ({ ...p, subtipo_graduacao: opt }))}
                                style={{ padding: '7px 16px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.8rem', border: 'none',
                                  background: (unifFormData.subtipo_graduacao || 'Corda') === opt ? 'rgba(124,58,237,0.45)' : 'rgba(255,255,255,0.07)',
                                  color: (unifFormData.subtipo_graduacao || 'Corda') === opt ? '#c4b5fd' : 'rgba(255,255,255,0.5)' }}>
                                {opt}
                              </button>
                            ))}
                          </div>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                          <label style={{ display: 'block', fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginBottom: 3 }}>Tamanho</label>
                          <input value={unifFormData.tamanho || ''} onChange={e => setUnifFormData((p: any) => ({ ...p, tamanho: e.target.value }))} placeholder="Ex: 80cm, M, 2,5m..."
                            style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
                        </div>
                      </div>
                    )}

                    {unifMsg && <div style={{ marginBottom: 8, fontSize: '0.78rem', color: unifMsg.startsWith('✅') ? '#4ade80' : '#f87171', fontWeight: 600 }}>{unifMsg}</div>}
                    <button disabled={unifSaving} onClick={addUniforme}
                      style={{ width: '100%', padding: '10px', background: '#064E3B', border: 'none', borderRadius: 8, color: '#fff', fontWeight: 700, cursor: unifSaving ? 'wait' : 'pointer', fontSize: '0.85rem', opacity: unifSaving ? 0.7 : 1 }}>
                      {unifSaving ? 'Enviando...' : '📤 Enviar Solicitação'}
                    </button>
                  </div>
                )}

                {unifMsg && !showUnifForm && (
                  <div style={{ marginBottom: 14, padding: '8px 12px', background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 8, fontSize: '0.78rem', color: '#4ade80', fontWeight: 600 }}>{unifMsg}</div>
                )}

                {/* Items list */}
                {ficha.uniformes.length === 0 && !showUnifForm && (
                  <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.35)', padding: '24px 0', fontSize: '0.85rem' }}>Nenhuma solicitação registrada.</div>
                )}
                {ficha.uniformes.length > 0 && (() => {
                  const cordaItems = ficha.uniformes.filter(u => (u as any).tipo_uniforme === 'corda');
                  const regularItems = ficha.uniformes.filter(u => (u as any).tipo_uniforme !== 'corda');
                  const total = ficha.uniformes.reduce((s, u) => s + (u.valor_unitario || 0) * (u.quantidade || 1), 0);
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {/* Graduation cord items */}
                      {cordaItems.length > 0 && (
                        <div style={{ background: 'rgba(124,58,237,0.12)', border: '1.5px solid rgba(124,58,237,0.3)', borderRadius: 10, padding: '12px 14px', marginBottom: 4 }}>
                          <div style={{ fontWeight: 700, fontSize: '0.75rem', color: '#a78bfa', marginBottom: 8 }}>🎗️ Graduação</div>
                          {cordaItems.map((u, i) => (
                            <div key={u.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.82rem', padding: '5px 0', borderBottom: i < cordaItems.length - 1 ? '1px solid rgba(124,58,237,0.2)' : 'none' }}>
                              <div><span style={{ fontWeight: 600 }}>{u.descricao}</span>{u.tamanho && <span style={{ color: '#a78bfa', marginLeft: 10 }}>📐 {u.tamanho}</span>}</div>
                      ...
                      {regularItems.map(u => (
                        <div key={u.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '12px 14px' }}>
                          ...
                          <div style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.78rem' }}>
                            {u.quantidade}× {u.valor_unitario > 0 ? formatMoeda(u.valor_unitario) + ' · Total: ' + formatMoeda(u.quantidade * u.valor_unitario) + ' · ' : ''}Solicitado em {formatDate(u.data_solicitacao)}
                          </div>
                        </div>
                      ))}
                      {total > 0 && (
                        <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 4 }}>
                          <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#4ade80', background: 'rgba(74,222,128,0.1)', borderRadius: 8, padding: '4px 12px', border: '1px solid rgba(74,222,128,0.25)' }}>Total: {formatMoeda(total)}</span>
                        </div>
                      )}
                    </div>
                  );
