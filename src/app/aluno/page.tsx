...
import { graduacoesInfantil, graduacoesAdulta, nomenclaturaGraduacao, getCordaColors } from '@/lib/graduacoes';

function computeIsMinor(data_nascimento?: string | null, menor_de_idade?: boolean): boolean {
  if (data_nascimento) {
    const dob = new Date(data_nascimento + 'T12:00:00');
...
type Tab = 'dashboard' | 'carteirinha' | 'presenca' | 'financeiro' | 'graduacao' | 'justificativas' | 'fotos' | 'playlist' | 'conta' | 'evolucao' | 'dados' | 'termo' | 'docs' | 'portfolio';
...
const GRAD_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  'Azul': { bg: '#f0fdf4', text: '#1e40af', border: '#60a5fa' },
...
export default function AlunoPage() {
  ...
  // Whether ANY account still has needs_password_reset (controls notice banner visibility).
  // Starts as true so banner always shows; only goes false when API confirms zero pending.
  const [hasPendingResets, setHasPendingResets] = useState<boolean>(true);
  ...
  const [eventoParticipantesOpen, setEventoParticipantesOpen] = useState<Set<string>>(new Set());
  ...
  // ── Portfolio ──────────────────────────────────────────────────────────────
  type PortfolioItem = { id: string; name: string; size: number; mime: string; uploaded_at: string; url: string | null };
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setLoading(false);
        // Se este dispositivo já gravou que o aluno redefiniu a senha, oculta o aviso
        try {
          if (localStorage.getItem('accbm_senha_redefinida') === '1') {
            setHasPendingResets(false);
          }
        } catch {}
      }
  ...
  const loadStudentData = useCallback(async (student_id: string, showGlobalLoader = false) => {
    ...
    try {
      const res = await fetch(`/api/aluno/dados?student_id=${encodeURIComponent(student_id)}`);
      if (res.ok) {
        ...
        if (studentData) {
          ...
          // Reset dadosInitialized so the form picks up fresh full data
          setDadosInitialized(false);
          // Fetch display ID for carteirinha (gerar-id is the authoritative source)
          const ordNum = (studentData as Record<string, unknown>).ordem_inscricao as number | null ?? null;
          if (ordNum) {
        ...
        } else if (showGlobalLoader) {
          // Student_id from session has no matching DB record — orphaned auth entry
          // Clear session so user sees login screen with helpful message instead of blank page
    ...
    } catch { /* silent — student may still be available from login response */ }
  ...
  useEffect(() => {
    if (session) {
      ...
      if (activeTab === 'graduacao') {
        ...
        fetch(`/api/eventos?student_id=${encodeURIComponent(session.student_id)}`, { cache: 'no-store' })
          .then(r => r.json())
          .then(d => { setEventos(Array.isArray(d) ? d : []); setEventosLoading(false); })
      ...
      if (activeTab === 'portfolio') {
        ...
        fetch('/api/admin/portfolio').then(r => r.json()).then(d => { setPortfolioItems(Array.isArray(d) ? d : []); setPortfolioLoading(false); }).catch(() => setPortfolioLoading(false));
      }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoginAttempts(0);
      // Guard: if login returned no student data, the account's student_id is orphaned (deleted student)
      if (!data.student) {
        setLoginError('Dados do aluno não encontrados. Entre em contato com a administração para verificar seu cadastro.');
      ...
      setSession(sess);
      // If account was reconstructed with temp password, force password reset before entering
      if (data.needs_password_reset) {
        setMustResetPassword(true);
        setResetPwForm({ new_password: '', confirm: '' });
        setResetPwError('');
      } else {
        // Student has their own password — hide the notice for this device
        try { localStorage.setItem('accbm_senha_redefinida', '1'); } catch {}
        setHasPendingResets(false);
  ...
  const handlePresenca = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      try {
        ...
        else if (data.notTrainingDay) { setPresencaMsg(data.message || 'Hoje não é dia de treino para seu núcleo.'); setPresencaStatus('error'); }
        else if (data.tooFar) { setPresencaMsg(data.message || 'Você está longe demais do local de treino. Aproxime-se (100m) para registrar presença.'); setPresencaStatus('error'); }
  ...
  // Populate dados form when FULL student data is loaded (studentLoaded=true) or when tab is activated after load
  useEffect(() => {
    // Only initialize from full server data (studentLoaded), not from partial login response
    if (student && studentLoaded && (activeTab === 'dados' || !dadosInitialized)) {
      setDadosForm({
  ...
  }, [student, studentLoaded, activeTab, dadosInitialized]);
  ...
  const cartData = student ? {
    ...
    foto_url: student.foto_url || null,
    menor_de_idade: computeIsMinor(student.data_nascimento, student.menor_de_idade),
    nome_pai: student.nome_pai as string || '',
    nome_mae: student.nome_mae as string || '',
    nome_responsavel: student.nome_responsavel as string || '',
    cpf_responsavel: student.cpf_responsavel as string || '',
  ...
  // ── FORCE PASSWORD RESET (reconstructed accounts) ─────────────────────────
  if (session && mustResetPassword) {
    const studentName = student?.nome_completo?.split(' ')[0] || 'Aluno';
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #064E3B 0%, #065f46 50%, #064E3B 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: '32px 28px', width: '100%', maxWidth: 420, boxShadow: '0 25px 60px rgba(0,0,0,0.4)' }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            <div style={{ fontSize: '2.5rem', marginBottom: 8 }}>🔐</div>
            <h2 style={{ margin: '0 0 8px', fontSize: '1.3rem', fontWeight: 800, color: '#111827' }}>Crie uma nova senha</h2>
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#6b7280', lineHeight: 1.5 }}>
              Olá, <strong>{studentName}</strong>! Por segurança, você precisa criar uma nova senha pessoal para continuar acessando sua conta.
            </p>
          </div>
          <div style={{ background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 10, padding: '12px 14px', marginBottom: 20, fontSize: '0.8rem', color: '#92400e' }}>
            ⚠️ Seu acesso foi restaurado pelo sistema. Defina uma senha nova e segura agora.
          </div>
          <form onSubmit={async (e) => {
            e.preventDefault();
            setResetPwError('');
            if (resetPwForm.new_password.length < 6) { setResetPwError('A senha deve ter pelo menos 6 caracteres.'); return; }
            if (resetPwForm.new_password !== resetPwForm.confirm) { setResetPwError('As senhas não coincidem.'); return; }
            setResetPwLoading(true);
            try {
              const res = await fetch('/api/aluno/auth', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                // Use reset-password (admin-like) which does NOT require current_password
                // This works regardless of whether current password is still 'accbm2025' or not
                body: JSON.stringify({ action: 'reset-password', student_id: session.student_id, new_password: resetPwForm.new_password }),
              });
              const data = await res.json();
              if (data.success) {
                setMustResetPassword(false);
                setActiveTab('dashboard');
                // Marca neste dispositivo que este aluno já criou sua senha
                try { localStorage.setItem('accbm_senha_redefinida', '1'); } catch {}
                setHasPendingResets(false);
              } else {
                setResetPwError(data.error || 'Erro ao salvar. Tente novamente.');
              }
            } catch { setResetPwError('Erro de conexão. Tente novamente.'); }
            finally { setResetPwLoading(false); }
          }}>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#374151', marginBottom: 6 }}>Nova senha *</label>
              <input type="password" value={resetPwForm.new_password} onChange={e => setResetPwForm(f => ({ ...f, new_password: e.target.value }))}
                placeholder="Mínimo 6 caracteres"
                style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: '1.5px solid #d1d5db', fontSize: '0.95rem', boxSizing: 'border-box', outline: 'none' }} required />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#374151', marginBottom: 6 }}>Confirmar nova senha *</label>
              <input type="password" value={resetPwForm.confirm} onChange={e => setResetPwForm(f => ({ ...f, confirm: e.target.value }))}
                placeholder="Repita a senha"
                style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: '1.5px solid #d1d5db', fontSize: '0.95rem', boxSizing: 'border-box', outline: 'none' }} required />
            </div>
            {resetPwError && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: '0.83rem', color: '#dc2626' }}>{resetPwError}</div>}
            <button type="submit" disabled={resetPwLoading}
              style={{ width: '100%', padding: '13px', borderRadius: 12, background: '#065f46', color: '#fff', border: 'none', fontSize: '1rem', fontWeight: 700, cursor: resetPwLoading ? 'not-allowed' : 'pointer', opacity: resetPwLoading ? 0.7 : 1 }}>
              {resetPwLoading ? 'Salvando...' : '✅ Salvar nova senha e entrar'}
            </button>
          </form>
          <button onClick={handleLogout} style={{ marginTop: 14, width: '100%', background: 'none', border: 'none', color: '#9ca3af', fontSize: '0.8rem', cursor: 'pointer', textDecoration: 'underline' }}>Sair</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #064E3B 0%, #065f46 50%, #064E3B 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          {/* ── Aviso de senha — desaparece somente quando todos tiverem criado senha ── */}
          {hasPendingResets && <>
            <style>{`
              @keyframes sirenGlow {
                0%, 100% {
              ...
              @keyframes sirenFlashRed {
                0%, 100% {
                  color: #ef4444;
                  text-shadow: 0 0 10px rgba(239,68,68,1), 0 0 25px rgba(239,68,68,0.8), 0 0 50px rgba(239,68,68,0.5);
                  filter: drop-shadow(0 0 8px rgba(239,68,68,0.9));
                }
                50% {
                  color: #f59e0b;
                  text-shadow: 0 0 10px rgba(245,158,11,1), 0 0 25px rgba(245,158,11,0.8), 0 0 50px rgba(245,158,11,0.5);
            ...
            `}</style>
            <div style={{ background: 'linear-gradient(135deg,rgba(254,243,199,0.98),rgba(255,237,213,0.98))', border: '2.5px solid #f59e0b', borderRadius: 16, padding: '18px 20px', marginBottom: 16, animation: 'sirenGlow 1.2s ease-in-out infinite' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: '1.9rem', animation: 'sirenFlashRed 0.6s ease-in-out infinite, sirenSpin 1.2s ease-in-out infinite', display: 'inline-block', lineHeight: 1, flexShrink: 0 }}>🚨</span>
                <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#92400e', flex: 1, textAlign: 'center', lineHeight: 1.5 }}>
                  Clique em &lsquo;Esqueci minha senha&rsquo; e cadastre uma nova para acessar a área do aluno.
                </div>
                <span style={{ fontSize: '1.9rem', animation: 'sirenFlashRed 0.6s ease-in-out infinite 0.3s, sirenSpin 1.2s ease-in-out infinite reverse', display: 'inline-block', lineHeight: 1, flexShrink: 0 }}>🚨</span>
              </div>
            </div>
          </>}
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '32px 28px', boxShadow: '0 25px 60px rgba(0,0,0,0.4)' }}>
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button type="submit" disabled={loginLoading || Date.now() < lockedUntil}
                style={{ background: loginLoading ? '#9ca3af' : '#064E3B', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 700, fontSize: '0.95rem', cursor: loginLoading ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
  ...
  if (showRegister) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#064E3B,#065f46)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: '28px 24px', width: '100%', maxWidth: 440, boxShadow: '0 25px 60px rgba(0,0,0,0.4)' }}>
          ...
          <div style={{ background: '#f0fdf4', border: '1px solid #6ee7b7', borderRadius: 10, padding: '9px 13px', marginBottom: 14, fontSize: '0.75rem', color: '#064E3B', lineHeight: 1.6 }}>
          ...
          <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
            ...
            <button
              ...
              style={{ background: registerLoading ? '#9ca3af' : registerSuccess ? '#16a34a' : '#064E3B', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 700, fontSize: '0.9rem', cursor: registerLoading || registerSuccess ? 'not-allowed' : 'pointer', marginTop: 4 }}>
  ...
  if (showForgot) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#064E3B,#065f46)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          ...
          {forgotStep === 'done' ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <button onClick={() => { setShowForgot(false); setForgotStep('lookup'); setForgotInput(''); setForgotMsg(''); setResetMsg(''); setResetPassword(''); setResetConfirmPassword(''); }}
                style={{ background: '#064E3B', color: '#fff', border: 'none', borderRadius: 10, padding: '12px 32px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer' }}>
                Entrar agora
              </button>
            </div>
          ) : forgotStep === 'reset' ? (
            <>
              ...
              <form onSubmit={async (e) => {
                ...
                const res = await fetch('/api/aluno/auth', {
                  method: 'POST', headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ action: 'reset-password', student_id: forgotStudentId, new_password: resetPassword }),
                });
                const data = await res.json();
                ...
                <button type="submit" disabled={forgotLoading || resetPassword.length < 6 || resetPassword !== resetConfirmPassword}
                  style={{ background: (resetPassword.length >= 6 && resetPassword === resetConfirmPassword) ? '#064E3B' : '#e5e7eb', color: (resetPassword.length >= 6 && resetPassword === resetConfirmPassword) ? '#fff' : '#9ca3af', border: 'none', borderRadius: 10, padding: 13, fontWeight: 700, fontSize: '0.95rem', cursor: forgotLoading ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 22 }}>
                ...
                <p style={{ margin: 0, fontSize: '0.78rem', color: '#6b7280' }}>Informe seu e-mail ou usuário para localizar sua conta e redefinir a senha.</p>
              </div>
              ...
              <form onSubmit={async (e) => {
                ...
                <button type="submit" disabled={forgotLoading || !forgotInput.trim()}
                  style={{ background: forgotInput.trim() ? '#064E3B' : '#e5e7eb', color: forgotInput.trim() ? '#fff' : '#9ca3af', border: 'none', borderRadius: 10, padding: 13, fontWeight: 700, fontSize: '0.95rem', cursor: forgotLoading ? 'not-allowed' : 'pointer' }}>
  ...
  const tabs: { id: Tab; label: string; badge?: boolean }[] = [
    ...
    { id: 'dados',          label: 'Dados', badge: !!(student && (!student.nucleo || !student.graduacao || !student.email)) },
    { id: 'termo',          label: 'Termo', badge: !!(student && student.menor_de_idade && !student.assinatura_responsavel) },
    ...
    { id: 'docs',           label: 'Docs' },
    { id: 'justificativas', label: 'Justif.' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'system-ui, -apple-system, sans-serif', overflowX: 'hidden', maxWidth: '100vw' }}>

      {/* Admin preview banner */}
      {isAdminPreview && (
        <div style={{ background: '#064E3B', color: '#fff', fontSize: '0.75rem', padding: '6px 20px', textAlign: 'center', fontWeight: 600, letterSpacing: '0.01em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      ...
      {/* Header */}
      <div style={{ position: 'sticky', top: 0, zIndex: 60, width: '100%', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
        <div style={{ background: nucleoColor, padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#fff' }}>
            {student?.foto_url
              ? <img src={student.foto_url} alt={displayName} style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.5)' }} />
            ...
            <div>
              ...
              <div style={{ fontSize: '0.72rem', opacity: 0.85 }}>{student?.nucleo || 'ACCBM'} • {student?.graduacao || 'Aluno'}</div>
            </div>
          </div>
          {isAdminPreview
            ? <span style={{ background: 'rgba(255,255,255,0.25)', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 8, padding: '5px 12px', color: '#fff', fontSize: '0.72rem', fontWeight: 700 }}>🔭 Visualização</span>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: '#fff', borderBottom: '1px solid #e5e7eb', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <style>{`
          .tab-scroll-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;}
          .tab-scroll-wrap::-webkit-scrollbar{display:none;}
          .tab-grid{display:flex;flex-direction:row;max-width:720px;margin:0 auto;padding:0 4px;min-width:max-content;}
          @media(min-width:600px){.tab-grid{display:grid;grid-template-columns:repeat(13,1fr);min-width:0;}}
          .tab-btn{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;padding:7px 8px;border:none;background:none;cursor:pointer;transition:all 0.15s;-webkit-tap-highlight-color:transparent;min-width:52px;}
          .tab-btn span{font-size:0.6rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:52px}
          @media(min-width:600px){.tab-btn{padding:7px 2px;min-width:0;}}
        `}</style>
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} className="tab-btn" onClick={() => setActiveTab(tab.id)}
                style={{ borderBottom: active ? `2.5px solid ${nucleoColor}` : '2.5px solid transparent', color: active ? nucleoColor : '#6b7280', fontWeight: active ? 700 : 400 }}>
                <span style={{ lineHeight: 1, fontSize: '1.2rem' }}>{TAB_ICONS[tab.id]}</span>
                <span style={{ fontSize: '0.58rem' }}>{tab.label}</span>
                {tab.badge && <span style={{ position: 'absolute', top: 4, right: '15%', width: 7, height: 7, borderRadius: '50%', background: '#ef4444', border: '1.5px solid #fff' }} />}
              </button>
            );
          })}
        </div>
      ...
      {/* Content */}
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px 40px', overflowX: 'hidden' }}>
        ...
        {activeTab === 'dashboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {cadastroIncompleto && (
                <div style={{ marginTop: 14, borderRadius: 14, overflow: 'hidden', border: '2px solid rgba(255,255,255,0.5)', boxShadow: '0 4px 18px rgba(0,0,0,0.25)', animation: 'pulseCard 1.6s ease-in-out infinite' }}>
                  <style>{`
                    ...
                    @keyframes pulseAlert { 0%,100%{transform:scale(1);opacity:1} 50%{transform:scale(1.4);opacity:0.7} }
                  `}</style>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
              {([
                { tab: 'carteirinha'    as Tab, icon: '🪪', label: 'Carteirinha',   color: '#f0fdf4', iconBg: '#d1fae5', textColor: '#1e40af' },
                ...
                { tab: 'portfolio'      as Tab, icon: '🎨', label: 'Portfólio',      color: '#f0fdf4', iconBg: '#bbf7d0', textColor: '#065f46' },
              ] as { tab: Tab; icon: string; label: string; color: string; iconBg: string; textColor: string }[]).map(item => {
                // Show alert dot on Financeiro if there are pending uniform requests
                const hasPendingUnif = item.tab === 'financeiro' && (fichaFin as any)?.uniformes?.some((u: any) => u.status === 'solicitado');
                return (
                <button key={item.tab} onClick={() => setActiveTab(item.tab)}
                  style={{ background: item.color, borderRadius: 14, padding: '14px 10px', border: 'none', cursor: 'pointer', textAlign: 'center', transition: 'transform 0.15s, box-shadow 0.15s', position: 'relative' }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: item.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', margin: '0 auto 8px', position: 'relative' }}>
                    {item.icon}
                    {hasPendingUnif && <span style={{ position: 'absolute', top: -3, right: -3, width: 12, height: 12, background: '#f59e0b', borderRadius: '50%', border: '2px solid #fff', animation: 'pulseAlert 1.5s ease-in-out infinite' }} />}
                  </div>
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: '1.6rem', filter: 'drop-shadow(0 0 8px rgba(255,255,255,0.8))', animation: 'pulse 1.5s ease-in-out infinite' }}>🔔</span>
                    <div>
                      <div style={{ color: '#fff', fontWeight: 900, fontSize: '1rem', letterSpacing: '0.03em', textShadow: '0 1px 4px rgba(0,0,0,0.3)' }}>
                        EVENTOS IMPORTANTES
                      </div>
                      <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.72rem', fontWeight: 600, marginTop: 1 }}>
                        {sorted.length} evento{sorted.length !== 1 ? 's' : ''} programado{sorted.length !== 1 ? 's' : ''} para você
                      </div>
                    </div>
                    <span style={{ marginLeft: 'auto', fontSize: '1.4rem', animation: 'bounce 1s infinite' }}>👇</span>
                  </div>
                  ...
                  {/* Lista de eventos — ordem cronológica */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    {sorted.slice(0, 3).map((ev: any) => {
                      const isUpcoming = ev.data ? new Date() < new Date(ev.data + 'T23:59:59') : false;
                      const hasParticipantes = ev.participantes && ev.participantes.length > 0;
                      const isOpen = eventoParticipantesOpen.has(ev.id);
                      return (
                        <div key={ev.id} style={{ background: '#fff', borderRadius: 16, border: `2px solid ${isUpcoming ? '#f59e0b' : '#e5e7eb'}`, boxShadow: isUpcoming ? '0 4px 18px rgba(245,158,11,0.2)' : '0 1px 6px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                          {/* Cover image */}
                          {ev.imagem_url && (
                            <img src={ev.imagem_url} alt={ev.nome} style={{ width: '100%', height: 'auto', display: 'block' }} />
                          )}
                          <div style={{ padding: '16px 18px' }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                              {!ev.imagem_url && (
                                <div style={{ width: 44, height: 44, borderRadius: 12, background: ev.tipo === 'batizado' ? '#fef3c7' : '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0 }}>
                                  {ev.tipo === 'batizado' ? '🥋' : '🎖️'}
                                </div>
                              )}
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#111827', lineHeight: 1.3 }}>
                                  {isUpcoming && <span style={{ marginRight: 5 }}>👉</span>}{ev.nome}
                                </div>
                                <div style={{ fontSize: '0.78rem', color: '#f59e0b', fontWeight: 700, marginTop: 4 }}>
                                  📅 {ev.data ? new Date(ev.data + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) : '—'}
                                  {ev.hora ? ` · ${ev.hora}` : ''}
                                </div>
                                {ev.local && <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: 2 }}>📍 {ev.local}</div>}
                                {ev.descricao && <div style={{ fontSize: '0.78rem', color: '#374151', marginTop: 6, whiteSpace: 'pre-line', lineHeight: 1.5 }}>{ev.descricao}</div>}
                                {ev.video_url && (
                                  <a href={ev.video_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 8, fontSize: '0.75rem', fontWeight: 700, color: '#2563eb', textDecoration: 'none', background: '#eff6ff', borderRadius: 8, padding: '5px 12px', border: '1px solid #bfdbfe' }}>
                                    🎬 Ver vídeo
                                  </a>
                                )}
                              </div>
                              <span style={{ flexShrink: 0, background: ev.tipo === 'batizado' ? '#fef3c7' : '#ede9fe', color: ev.tipo === 'batizado' ? '#92400e' : '#5b21b6', borderRadius: 8, padding: '4px 10px', fontSize: '0.7rem', fontWeight: 700 }}>
                                {ev.tipo === 'batizado' ? 'Batizado' : 'Troca'}
                              </span>
                            </div>

                            {/* Relação de alunos — collapsible */}
                            {hasParticipantes && (
                              <div style={{ marginTop: 14 }}>
                                <button
                                  onClick={() => setEventoParticipantesOpen(prev => {
                                    const next = new Set(prev);
                                    if (next.has(ev.id)) next.delete(ev.id); else next.add(ev.id);
                                    return next;
                                  })}
                                  style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(139,92,246,0.08))', border: '1.5px solid rgba(99,102,241,0.25)', borderRadius: 10, cursor: 'pointer', color: '#4f46e5', fontWeight: 800, fontSize: '0.82rem' }}>
                                  <span>🎓 Relação de Alunos — {ev.tipo === 'batizado' ? 'Batizado' : 'Troca de Graduação'} ({ev.participantes.length})</span>
                                  <span style={{ fontSize: '1rem', transition: 'transform 0.2s', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
                                </button>
                                {isOpen && (
                                  <div style={{ marginTop: 8, borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(99,102,241,0.2)' }}>
                                    <div style={{ background: 'rgba(99,102,241,0.06)', padding: '8px 14px', fontSize: '0.68rem', fontWeight: 800, color: '#6366f1', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8 }}>
                                      <span>Aluno</span><span>Atual</span><span>Nova</span>
                                    </div>
                                    {ev.participantes.map((p: any, i: number) => (
                                      <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8, padding: '9px 14px', background: i % 2 === 0 ? '#fff' : '#f9fafb', alignItems: 'center', borderTop: '1px solid #f3f4f6' }}>
                                        <span style={{ fontWeight: 600, fontSize: '0.82rem', color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.nome_completo}</span>
                                        <span style={{ fontSize: '0.72rem', color: '#f59e0b', fontWeight: 700, whiteSpace: 'nowrap', background: '#fef3c7', borderRadius: 6, padding: '2px 7px' }}>{p.graduacao_atual}</span>
                                        <span style={{ fontSize: '0.72rem', color: '#16a34a', fontWeight: 700, whiteSpace: 'nowrap', background: '#dcfce7', borderRadius: 6, padding: '2px 7px' }}>{p.nova_graduacao || '—'}</span>
                                      </div>
                                    ))}
                                  </div>
                                )}
                    ...
                    {sorted.length > 3 && (
                      <div style={{ textAlign: 'center', fontSize: '0.75rem', color: '#9ca3af' }}>+{sorted.length - 3} evento(s) — veja na aba Graduação</div>
                    )}
                  </div>
                </div>
              );
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {([
                ...
                { href: '/organograma', icon: '🏛️', label: 'Organograma da ACCBM', color: '#f0fdf4', iconBg: '#a7f3d0', textColor: '#1e40af' },
        ...
        {activeTab === 'carteirinha' && cartData && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button
              ...
              style={{ width: '100%', padding: '12px', background: '#064E3B', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
            >
        ...
        {activeTab === 'financeiro' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {!fichaFinLoading && (() => {
              ...
              const hasPendingUnif = fin ? (fin.uniformes as any[] || []).some((u: any) => u.status === 'solicitado') : false;
              const mensalidadesPendentes = fin ? (fin.mensalidades as any[] || []).filter((m: any) => m.status !== 'pago').length : 0;
              const batizadoPendente = fin?.batizado?.modalidade !== 'nao_definido' && fin?.batizado?.status_geral !== 'pago';
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {fin && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                      <div style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                        <div style={{ fontSize: '1.3rem', marginBottom: 4 }}>🥋</div>
                        <div style={{ fontSize: '0.7rem', color: '#6b7280', fontWeight: 600, marginBottom: 2 }}>Batizado</div>
                        <div style={{ fontSize: '0.75rem', fontWeight: 800, color: batizadoPendente ? '#d97706' : fin?.batizado?.modalidade === 'nao_definido' ? '#9ca3af' : '#16a34a' }}>
                          {fin?.batizado?.modalidade === 'nao_definido' ? '—' : batizadoPendente ? 'Pendente' : '✓ Pago'}
                        </div>
                      </div>
                      <div style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                        <div style={{ fontSize: '1.3rem', marginBottom: 4 }}>📅</div>
                        <div style={{ fontSize: '0.7rem', color: '#6b7280', fontWeight: 600, marginBottom: 2 }}>Mensalidades</div>
                        <div style={{ fontSize: '0.75rem', fontWeight: 800, color: mensalidadesPendentes > 0 ? '#d97706' : '#9ca3af' }}>
                          {mensalidadesPendentes > 0 ? `${mensalidadesPendentes} pendente${mensalidadesPendentes > 1 ? 's' : ''}` : (fin.mensalidades?.length > 0 ? '✓ Em dia' : '—')}
                        </div>
                      </div>
                      <div style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', border: '1px solid #e5e7eb', textAlign: 'center', position: 'relative' }}>
                        {hasPendingUnif && <span style={{ position: 'absolute', top: 8, right: 8, width: 8, height: 8, borderRadius: '50%', background: '#f59e0b' }} />}
                        <div style={{ fontSize: '1.3rem', marginBottom: 4 }}>👕</div>
                        <div style={{ fontSize: '0.7rem', color: '#6b7280', fontWeight: 600, marginBottom: 2 }}>Uniformes</div>
                        <div style={{ fontSize: '0.75rem', fontWeight: 800, color: hasPendingUnif ? '#d97706' : '#9ca3af' }}>
                          {fin.uniformes?.length > 0 ? (hasPendingUnif ? 'Pendente' : '✓ Atualizado') : '—'}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {/* CTA card */}
                  <div style={{ background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', borderRadius: 14, padding: '20px 18px', border: '1px solid #bbf7d0', textAlign: 'center' }}>
                    <div style={{ fontSize: '2rem', marginBottom: 8 }}>💰</div>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#065f46', marginBottom: 6 }}>Ficha Financeira Completa</div>
                    <div style={{ fontSize: '0.82rem', color: '#047857', marginBottom: 16 }}>
                      Acesse o portal completo para gerenciar mensalidades, batizado, contribuições, uniformes e enviar comprovantes de pagamento.
                    </div>
                    <a href={`/financeiro?student_id=${session?.student_id}`} target="_blank" rel="noreferrer"
                      style={{ display: 'inline-block', background: 'linear-gradient(135deg,#16a34a,#15803d)', color: '#fff', textDecoration: 'none', borderRadius: 10, padding: '12px 28px', fontSize: '0.92rem', fontWeight: 800, letterSpacing: '0.01em' }}>
        ...
        {activeTab === 'graduacao' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Próximos eventos — aba Graduação */}
            {(eventosLoading || eventos.length > 0) && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {!eventosLoading && [...eventos].sort((a, b) => (a.data || '').localeCompare(b.data || '')).map((ev: any) => {
                  const isUpcoming = ev.data ? new Date() < new Date(ev.data + 'T23:59:59') : false;
                  const hasParticipantes = ev.participantes && ev.participantes.length > 0;
                  const isOpen = eventoParticipantesOpen.has(ev.id);
                  return (
                  <div key={ev.id} style={{ background: '#fff', borderRadius: 16, border: `2px solid ${isUpcoming ? '#f59e0b' : '#e5e7eb'}`, boxShadow: isUpcoming ? '0 4px 18px rgba(245,158,11,0.2)' : '0 1px 6px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '16px 18px' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        ...
                        <div style={{ flexShrink: 0, background: ev.tipo === 'batizado' ? '#fef3c7' : '#ede9fe', color: ev.tipo === 'batizado' ? '#92400e' : '#5b21b6', borderRadius: 8, padding: '4px 10px', fontSize: '0.72rem', fontWeight: 700 }}>
            ...
            {student?.graduacao && (() => {
              ...
              const cordColors = getCordaColors(student.graduacao);
              const latestFinalizado = historico.find(r => r.status === 'finalizado' || !r.status);
              return (
                <div style={{ background: c.bg, border: `2px solid ${c.border}`, borderRadius: 16, overflow: 'hidden' }}>
                  {/* Cord color bar */}
                  <div style={{ height: 8, display: 'flex' }}>
                    {cordColors.map((color, i) => (
                      <div key={i} style={{ flex: 1, background: color }} />
                    ))}
                  </div>
                  <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
                    {/* Cord visual circle */}
                    <div style={{ width: 56, height: 56, borderRadius: '50%', border: `3px solid ${c.border}`, flexShrink: 0, position: 'relative', overflow: 'hidden', boxShadow: `0 0 0 4px ${c.border}30, 0 4px 14px ${c.border}40` }}>
                      {cordColors.length === 1 ? (
                        <div style={{ width: '100%', height: '100%', background: cordColors[0] }} />
                      ) : (
                        <div style={{ width: '100%', height: '100%', display: 'flex' }}>
                          <div style={{ flex: 1, background: cordColors[0] }} />
                          <div style={{ flex: 1, background: cordColors[1] }} />
                        </div>
                      )}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '0.7rem', color: c.text, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Graduação Atual</div>
                      <div style={{ fontSize: '1.2rem', fontWeight: 800, color: c.text }}>{student.graduacao}</div>
                      {student.tipo_graduacao && (() => {
                        const t = (student.tipo_graduacao as string || '').toLowerCase();
                        // tipo_graduacao is the explicit choice — respect it; only fall back to age if not set
                        const label = t === 'infantil' ? 'Infantil' : t === 'adulta' || t === 'adulto' ? 'Adulto'
                          : computeIsMinor(student.data_nascimento, student.menor_de_idade) ? 'Infantil' : 'Adulto';
                        return <div style={{ fontSize: '0.78rem', color: c.text, opacity: 0.7, marginTop: 1 }}>{label}</div>;
                      })()}
                      {/* Cord colors text */}
                      <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                        {cordColors.map((color, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <div style={{ width: 14, height: 14, borderRadius: '50%', background: color, border: '1.5px solid rgba(0,0,0,0.1)', flexShrink: 0 }} />
                            <span style={{ fontSize: '0.68rem', color: c.text, opacity: 0.7 }}>{color}</span>
                          </div>
                        ))}
                      </div>
                      {latestFinalizado && (
                        <div style={{ fontSize: '0.72rem', color: c.text, opacity: 0.6, marginTop: 4 }}>
                          📅 {new Date(latestFinalizado.data_graduacao + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}
                          {latestFinalizado.evento ? ` · ${latestFinalizado.evento}` : ''}
                        </div>
                      )}
                    </div>
                    <div style={{ flexShrink: 0, background: '#dcfce7', color: '#166534', borderRadius: 10, padding: '6px 12px', fontSize: '0.72rem', fontWeight: 800, textAlign: 'center' }}>
                      ✅<br/>Atual
                    </div>
                  </div>
                </div>
            ...
            {loadingHistorico ? (
              ...
              <div>
                <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
                  Histórico Completo — {historico.filter(r => !r.status || r.status === 'finalizado').length} graduação(ões) registrada(s)
                </div>
                <div style={{ position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                    {historico.map((reg, idx) => {
                      ...
                      const cordColors = getCordaColors(reg.graduacao_recebida);
                      const isFinalizado = !reg.status || reg.status === 'finalizado';
                      const isPendente = reg.status === 'pendente';
                      return (
                        <div key={reg.id} style={{ display: 'flex', gap: 14, paddingBottom: 20, paddingLeft: 4, opacity: isPendente ? 0.65 : 1 }}>
                          {/* Timeline dot — visual cord color */}
                          <div style={{ flexShrink: 0, width: 34, height: 34, borderRadius: '50%', border: `3px solid ${isPendente ? '#fde68a' : c.border}`, zIndex: 1, marginTop: 4, overflow: 'hidden', position: 'relative', boxShadow: idx === 0 && !isPendente ? `0 0 0 3px ${c.border}44, 0 2px 8px ${c.border}60` : '0 1px 4px rgba(0,0,0,0.15)' }}>
                            {isPendente ? (
                              <div style={{ width: '100%', height: '100%', background: '#fef9c3', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem' }}>⏳</div>
                            ) : cordColors.length === 1 ? (
                              <div style={{ width: '100%', height: '100%', background: cordColors[0] }} />
                            ) : (
                              // Split halves for two-color cords
                              <div style={{ width: '100%', height: '100%', display: 'flex' }}>
                                <div style={{ flex: 1, background: cordColors[0] }} />
                                <div style={{ flex: 1, background: cordColors[1] }} />
                              </div>
                            )}
                            {/* "Atual" crown indicator */}
                            {idx === 0 && !isPendente && (
                              <div style={{ position: 'absolute', top: -1, right: -1, width: 12, height: 12, borderRadius: '50%', background: '#fbbf24', border: '1.5px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.45rem' }}>★</div>
                            )}
                          </div>

                          {/* Card */}
                          <div style={{ flex: 1, background: '#fff', borderRadius: 14, border: `1px solid ${isPendente ? '#fde68a' : '#e5e7eb'}`, boxShadow: '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
                            {/* Cord color bar */}
                              ...
                              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                                <div style={{ flex: 1 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
                                    <span style={{ background: c.bg, border: `1px solid ${c.border}`, color: c.text, borderRadius: 8, padding: '3px 10px', fontSize: '0.82rem', fontWeight: 700 }}>
                                      {reg.graduacao_recebida}
                                    </span>
                                    {idx === 0 && isFinalizado && (
                                      <span style={{ background: '#dcfce7', color: '#166534', borderRadius: 8, padding: '2px 8px', fontSize: '0.7rem', fontWeight: 700 }}>
                                        ✅ Atual
                                      </span>
                                    )}
                                    {isPendente && (
                                      <span style={{ background: '#fef9c3', color: '#92400e', borderRadius: 8, padding: '2px 8px', fontSize: '0.7rem', fontWeight: 700 }}>
                                        ⏳ Pendente
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                                    {cordColors.map((color, i) => (
                                      <div key={i} style={{ width: 12, height: 12, borderRadius: '50%', background: color, border: '1px solid rgba(0,0,0,0.1)', flexShrink: 0 }} />
                                    ))}
                                    <span style={{ fontSize: '0.7rem', color: '#9ca3af' }}>Corda</span>
                                  </div>
                                  {reg.evento && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                                      <span style={{ fontSize: '0.8rem' }}>{reg.evento.toLowerCase().includes('batizado') ? '🥋' : '🎓'}</span>
                                      <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#111827' }}>{reg.evento}</span>
                                    </div>
                                  )}
                                  {reg.professor_responsavel && (
                                    <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                                      👨‍🏫 Prof. {reg.professor_responsavel}
                                    </div>
                                  )}
                                </div>
                                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                  <div style={{ fontSize: '0.78rem', fontWeight: 700, color: nucleoColor }}>
                                  ...
                                  {reg.finalizado_em && (
                                    <div style={{ fontSize: '0.65rem', color: '#9ca3af', marginTop: 2 }}>
                                      Aplicado {new Date(reg.finalizado_em).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
                                    </div>
                                  )}
                                </div>
                              </div>
                              {reg.observacoes && (
                                <div style={{ marginTop: 8, padding: '8px 10px', background: '#f9fafb', borderRadius: 8, fontSize: '0.76rem', color: '#6b7280', fontStyle: 'italic' }}>
            ...
            {historico.some(r => r.status === 'pendente') && (
              <div style={{ background: '#fffbeb', borderRadius: 12, padding: '12px 16px', border: '1px solid #fde68a', fontSize: '0.78rem', color: '#92400e', lineHeight: 1.5 }}>
                ⏳ Você possui registro(s) de graduação <strong>pendentes</strong> — serão aplicados quando o administrador finalizar o evento.
              </div>
            )}
            <div style={{ background: '#f0f9ff', borderRadius: 12, padding: '12px 16px', border: '1px solid #bae6fd', fontSize: '0.78rem', color: '#0369a1', lineHeight: 1.5 }}>
              ℹ️ O histórico é atualizado automaticamente quando o administrador finaliza um evento de batizado ou troca de corda no painel. Clique em <strong>Atualizar</strong> para ver novidades.
            </div>
          </div>
        )}
                  ...
                  {justificativas.map(j => (
                    <div key={j.id} style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        {j.resposta_mestre && (
                          <div style={{ marginTop: 6, padding: '6px 10px', background: '#f0fdf4', borderRadius: 6, fontSize: '0.76rem', color: '#064E3B' }}>
                    ...
                    <div style={{ padding: '8px 10px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                        <a href={m.url} download target="_blank" rel="noreferrer"
                          style={{ flex: 1, background: '#f0fdf4', color: '#064E3B', borderRadius: 6, padding: '4px 6px', fontSize: '0.68rem', fontWeight: 700, textDecoration: 'none', textAlign: 'center' }}>
                          ⬇ Baixar
                        </a>
        ...
        {activeTab === 'portfolio' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: '#111827' }}>🎨 Portfólio do Grupo</h2>
              <p style={{ margin: '4px 0 0', fontSize: '0.78rem', color: '#6b7280' }}>Arquivos e materiais do grupo disponibilizados pela administração.</p>
            </div>
            {portfolioLoading ? (
              <div style={{ textAlign: 'center', padding: 48, color: '#6b7280' }}>Carregando portfólio...</div>
            ) : portfolioItems.length === 0 ? (
              <div style={{ textAlign: 'center', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 14, padding: '48px 20px' }}>
                <div style={{ fontSize: 48, marginBottom: 12 }}>🎨</div>
                <div style={{ fontWeight: 700, color: '#374151', marginBottom: 4 }}>Nenhum arquivo disponível</div>
                <div style={{ color: '#6b7280', fontSize: '0.82rem' }}>A administração ainda não publicou arquivos no portfólio do grupo.</div>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 14 }}>
                {portfolioItems.map(item => {
                  ...
                  const icon = isImage ? '🖼️' : isPdf ? '📄' : item.mime?.includes('video') ? '🎬' : item.mime?.includes('audio') ? '🎵' : '📎';
                  const sizeLabel = item.size < 1024 * 1024 ? `${Math.round(item.size / 1024)} KB` : `${(item.size / (1024 * 1024)).toFixed(1)} MB`;
                  return (
                    <div key={item.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                      {isImage && item.url ? (
                        <img src={item.url} alt={item.name} style={{ width: '100%', height: 130, objectFit: 'cover' }} />
                      ) : (
                        <div style={{ height: 90, background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>{icon}</div>
                      )}
                      <div style={{ padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <div style={{ fontWeight: 700, fontSize: '0.83rem', color: '#111827', wordBreak: 'break-word' }}>{item.name}</div>
                        <div style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{sizeLabel} · {new Date(item.uploaded_at).toLocaleDateString('pt-BR')}</div>
                        {item.url && (
                          <a href={item.url} target="_blank" rel="noopener noreferrer"
                            style={{ display: 'block', textAlign: 'center', marginTop: 6, background: '#064E3B', color: '#fff', padding: '7px', borderRadius: 7, fontSize: '0.78rem', fontWeight: 700, textDecoration: 'none' }}>
                            Abrir / Baixar
                          </a>
                        )}
                      </div>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <button onClick={() => { setContaSection('edit-profile'); setContaForm(f => ({ ...f, new_username: session.username, new_email: student?.email || '' })); setContaMsg(''); }}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#f0fdf4', border: '1px solid #6ee7b7', borderRadius: 12, padding: '14px 16px', cursor: 'pointer', textAlign: 'left' }}>
                    <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#d1fae5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', flexShrink: 0 }}>✏️</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#064E3B' }}>Editar Perfil</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '18px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    }} style={{ background: '#064E3B', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 700, fontSize: '0.9rem', cursor: contaLoading ? 'wait' : 'pointer', opacity: contaLoading ? 0.7 : 1 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: embed ? '1px solid #f3f4f6' : 'none' }}>
                              ...
                              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                ...
                                <button onClick={() => { setPlaylistEditId(item.id); setPlaylistEditTitle(item.title); setPlaylistEditUrl(item.url); }}
                                  style={{ padding: '5px 8px', borderRadius: 7, background: '#f0fdf4', border: 'none', color: '#064E3B', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer' }}>✏️</button>
        ...
        {activeTab === 'dados' && session && !student && (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9ca3af' }}>
            ...
            <div style={{ fontSize: '0.9rem' }}>Carregando dados do aluno...</div>
          </div>
        )}
        {activeTab === 'dados' && session && student && (() => {
          // ── CPF validator ──────────────────────────────────────────────────
          ...
          const gradOpts = tipoEfetivo === 'Infantil' ? graduacoesInfantil : tipoEfetivo === 'Adulto' ? graduacoesAdulta : [...graduacoesInfantil, ...graduacoesAdulta];
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#f0fdf4', border: '1px solid #6ee7b7', borderRadius: 12, padding: '12px 16px', fontSize: '0.82rem', color: '#064E3B', fontWeight: 600, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <select value={dadosForm.graduacao} onChange={e => setDadosForm(p => ({ ...p, graduacao: e.target.value }))} style={fs}>
                      ...
                      {!tipoEfetivo && <>
                        <optgroup label="Graduação Infantil">{graduacoesInfantil.map(g => <option key={g} value={g}>{g}{nomenclaturaGraduacao[g] ? ` — ${nomenclaturaGraduacao[g]}` : ''}</option>)}</optgroup>
                        <optgroup label="Graduação Adulta">{graduacoesAdulta.map(g => <option key={g} value={g}>{g}{nomenclaturaGraduacao[g] ? ` — ${nomenclaturaGraduacao[g]}` : ''}</option>)}</optgroup>
                      </>}
                    </select>
                    {dadosForm.graduacao && (() => {
                      const cordColors = getCordaColors(dadosForm.graduacao);
                      return (
                        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <div style={{ height: 20, borderRadius: 8, overflow: 'hidden', display: 'flex', border: '1px solid var(--border)' }}>
                            {cordColors.map((c, i) => <div key={i} style={{ flex: 1, background: c }} />)}
                          </div>
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-primary)' }}>{dadosForm.graduacao}</span>
                            {nomenclaturaGraduacao[dadosForm.graduacao] && (
                              <span style={{ fontSize: '0.72rem', fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: 'rgba(6,78,59,0.1)', color: '#064E3B', border: '1px solid rgba(6,78,59,0.2)' }}>
                                {nomenclaturaGraduacao[dadosForm.graduacao]}
                              </span>
                            )}
                          </div>
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label style={{ ...ls, display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', userSelect: 'none' }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: '0.82rem', color: '#374151', lineHeight: 1.4 }}>
                        Autorizo o uso da minha imagem (ou do menor sob minha responsabilidade) em fotos, vídeos e demais registros realizados durante as atividades da Associação Cultural de Capoeira Barão de Mauá. A utilização será destinada exclusivamente para fins institucionais, educativos e de divulgação em mídias sociais, site, materiais promocionais e demais meios digitais e físicos, em território nacional e internacional, sem qualquer remuneração. A presente autorização é concedida por prazo indeterminado, enquanto perdurar o vínculo do aluno com a associação, podendo ser revogada a qualquer momento. A associação compromete-se a utilizar as imagens de forma ética, em conformidade com o Estatuto da Criança e do Adolescente (ECA) e a Lei Geral de Proteção de Dados (LGPD).
                      </span>
                    </label>
                  </div>
                  <div>
              ...
              <div style={{ background: '#fff', borderRadius: 16, padding: '18px 20px', border: '1px solid #e5e7eb', boxShadow: '0 1px 6px rgba(0,0,0,0.04)' }}>
                ...
                {(autoTipo === 'Infantil' || (student.menor_de_idade as boolean)) && (
                  <div style={{ marginTop: 10, background: '#f0fdf4', borderRadius: 8, padding: '8px 12px', fontSize: '0.75rem', color: '#064E3B' }}>
        ...
        {activeTab === 'termo' && session && student && (() => {
          ...
          const handlePrint = () => {
            ...
            w.document.write(`<!DOCTYPE html><html><head><title>Termo — ${student.nome_completo}</title><style>body{font-family:Georgia,serif;max-width:680px;margin:40px auto;padding:20px;color:#111}h1{text-align:center;font-size:1.3rem}p{line-height:1.9;text-align:justify}.box{background:#f9f9f9;border:1px solid #ccc;padding:14px 18px;border-radius:8px;margin-bottom:20px;font-family:sans-serif}.label{font-size:0.75rem;text-transform:uppercase;letter-spacing:0.06em;color:#666}.value{font-weight:700;font-size:0.95rem}.sig{margin-top:40px;display:flex;justify-content:space-between}.line{border-top:1px solid #333;width:260px;text-align:center;padding-top:6px;font-size:0.8rem;font-family:sans-serif}</style></head><body>
              <h1>Termo de Consentimento e Responsabilidade — Menor de Idade</h1>
              ...
              <p>Eu, <strong>${termoForm.nome_responsavel || '________________________'}</strong>, portador(a) do CPF <strong>${termoForm.cpf_responsavel || '___.___.___-__'}</strong>, responsável legal pelo menor <strong>${student.nome_completo}</strong>, autorizo sua participação nas atividades de capoeira realizadas pela <strong>Associação Cultural de Capoeira Barão de Mauá</strong>, declarando estar ciente das práticas físicas envolvidas.</p>
              <p>Declaro que o menor encontra-se em condições adequadas de saúde para participação nas atividades, responsabilizando-me pela veracidade e atualização das informações prestadas.</p>
              <p>Autorizo, de forma livre, informada e inequívoca, a coleta e o tratamento dos dados pessoais do menor e do responsável, incluindo nome completo, CPF, telefone e/ou e-mail e informações sociais, bem como dados de localização (GPS), coletados pontualmente no momento do registro de presença, exclusivamente para fins de confirmação de participação nas atividades. Não é realizado monitoramento contínuo de localização.</p>
              <p><strong>🔒 USO DE DADOS PESSOAIS (LGPD)</strong> — Os dados serão utilizados para: gestão e cadastro de alunos, controle de presença e frequência, comunicação com responsáveis, organização das atividades e elaboração de relatórios sociais e institucionais. Os dados serão utilizados exclusivamente pela administração da Associação Cultural de Capoeira Barão de Mauá, com acesso restrito à equipe responsável, não sendo divulgados publicamente. A associação adota medidas técnicas e administrativas para proteger os dados pessoais. O tratamento será realizado em conformidade com o <strong>Estatuto da Criança e do Adolescente (ECA)</strong> e a <strong>Lei Geral de Proteção de Dados (LGPD)</strong>. Este consentimento poderá ser revogado a qualquer momento mediante solicitação do responsável.</p>
              <div class="sig">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#fff', border: `2px solid ${nucleoColor}`, borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '24px 22px', fontFamily: 'Georgia, serif' }}>
                  ...
                  <div style={{ textAlign: 'justify', lineHeight: 1.9, marginBottom: 22, fontSize: '0.9rem' }}>
                    <p style={{ marginBottom: 12 }}>
                      Eu, responsável legal pelo menor acima identificado, autorizo sua participação nas atividades de capoeira realizadas pela <strong>Associação Cultural de Capoeira Barão de Mauá</strong>, declarando estar ciente das práticas físicas envolvidas.
                    </p>
                    <p style={{ marginBottom: 12 }}>
                    ...
                    <p style={{ marginBottom: 12 }}>
                      Autorizo, de forma livre, informada e inequívoca, a coleta e o tratamento dos dados pessoais do menor e do responsável, incluindo nome completo, CPF, telefone e/ou e-mail e informações sociais, bem como dados de localização (GPS), coletados pontualmente no momento do registro de presença, exclusivamente para fins de confirmação de participação nas atividades. <em>Não é realizado monitoramento contínuo de localização.</em>
                    </p>
                    <div style={{ background: 'rgba(220,38,38,0.04)', border: '1px solid rgba(220,38,38,0.15)', borderRadius: 8, padding: '10px 14px', fontSize: '0.85rem' }}>
                      <p style={{ fontWeight: 700, marginBottom: 6 }}>🔒 USO DE DADOS PESSOAIS (LGPD)</p>
                      ...
                      <p>
                        O tratamento dos dados será realizado em conformidade com o <strong>Estatuto da Criança e do Adolescente (ECA)</strong> e a <strong>Lei Geral de Proteção de Dados (LGPD)</strong>. Este consentimento poderá ser revogado a qualquer momento mediante solicitação do responsável.
                      </p>
                    </div>
                  </div>
