...
export default function PresencaPage() {
  ...
  const handleRegistrar = async (student: Student) => {
    ...
    // Se tem coords mas nenhum local dentro de 100m, tenta ampliar para 500m só para exibição (nome do local)
    if (coords && !local) {
      local = detectarLocal(coords.lat, coords.lng, 500) ?? null;
    }
    ...
    if ((result as any).notTrainingDay) {
      alert((result as any).message || 'Hoje não é dia de treino para este núcleo.');
      return;
    }
    if ((result as any).tooFar) {
      alert((result as any).message || 'Você está longe demais do local de treino. Aproxime-se para registrar presença.');
      return;
    }
    if (result.success) {
      setRegisteredToday(prev => new Set([...prev, student.id]));
      ...
      <div className="container" style={{ maxWidth: 580, marginTop: 28 }}>
        ...
        {!isOnline && (
          <div style={{ background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.5)', borderRadius: 10, padding: '10px 14px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>📶</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#d97706' }}>Sem conexão com internet</div>
        ...
        {offlineQueue.length > 0 && isOnline && (
          <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(6,78,59,0.25)', borderRadius: 10, padding: '10px 14px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span>☁️</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#064E3B' }}>{syncing ? 'Sincronizando...' : `${offlineQueue.length} presença(s) pendente(s) de sync`}</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Registradas offline e aguardando envio ao servidor.</div>
            </div>
            {!syncing && (
              <button onClick={() => syncOfflineQueue(offlineQueue)} style={{ background: 'rgba(6,78,59,0.15)', border: '1px solid rgba(6,78,59,0.3)', color: '#064E3B', padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.8rem' }}>
                Sincronizar
              </button>
            )}
          </div>
      ...
      {success && (
        <div className="modal-overlay" onClick={() => setSuccess(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: 440, textAlign: 'center' }}>
            ...
            {!isOnline && (
              <div style={{ background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.4)', borderRadius: 8, padding: '7px 12px', marginBottom: 16, fontSize: '0.78rem', color: '#d97706', fontWeight: 600 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {success.student.email && success.student.email.trim() ? (
                <a
                  ...
                  style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg,#064E3B,#059669)', border: 'none', color: '#fff', borderRadius: 12, fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, boxShadow: '0 4px 14px rgba(6,78,59,0.25)' }}
                >
