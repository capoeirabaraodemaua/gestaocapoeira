...
export default function PresencaPage() {
  ...
  const [success, setSuccess] = useState<{ student: Student; hora: string; data: string } | null>(null);
  ...
  const handleRegistrar = async (student: Student) => {
    ...
    if (result.success) {
      ...
      setSearch('');
      setFiltered([]);
      setSuccess({ student, hora, data });
    } else {
      alert('Erro ao registrar presença. Tente novamente.');
    }
  };

  const buildWhatsAppLink = (s: { student: Student; hora: string; data: string }) => {
          ...
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: 440, textAlign: 'center' }}>
            ...
            <div style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 12, padding: 18, textAlign: 'left', marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>Telefone</div>
                  <div style={{ fontWeight: 600, fontSize: '0.88rem' }}>{success.student.telefone || '—'}</div>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {success.student.telefone ? (
                ...
                <div style={{ flex: 1, padding: '12px', background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-secondary)', borderRadius: 10, fontSize: '0.82rem', textAlign: 'center' }}>
                  Sem telefone cadastrado
                </div>
              )}
              <button
                onClick={() => setSuccess(null)}
                style={{ flex: 1, padding: '12px', background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-secondary)', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: '0.9rem' }}
              >
                Concluir
              </button>
            </div>
